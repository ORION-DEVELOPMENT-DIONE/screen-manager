...
from . import config
import wiringpi
class Touch_1inch28(config.OrangePi):
    ...
    def WhoAmI(self):
        if (0xB5) != self.Touch_Read_Byte(0xA7):
            print('chip id is : ',[self.Touch_Read_Byte(0xA7)][0])
            return False
        return True


    def Configure_Standby(self, timeout=2):
        """
        Configure the touchscreen to enter standby mode automatically after a timeout.
        :param timeout: Timeout in seconds before entering standby mode.
        """
        try:
            # Enable auto-sleep by writing 0x00 to 0xFE
            #self.Touch_Write_Byte(0xFE, 0x00)
    ...
    def Touch_Reset(self):
        ...
        wiringpi.digitalWrite(self.TP_RST, 0)
        time.sleep(0.01)
        wiringpi.pinMode(self.TP_RST,1)
